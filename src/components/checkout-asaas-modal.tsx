...
// CPF validation helper function (reuse from existing modal)
...
interface CheckoutAsaasModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function CheckoutAsaasModal({
  open,
  onOpenChange,
}: CheckoutAsaasModalProps) {
  const { 
    createAsaasCheckout, 
    validateCoupon, 
    openBoleto,
  ...
  const handleCreateCheckout = async (values: FormValues) => {
    ...
    const checkoutResult = await createAsaasCheckout({
      ...
      userIdentification: {
        type: 'CPF',
        number: values.cpf.replaceAll(/\D/g, ''),
      },
      userPhone: {
        area_code: areaCode,
        number: phoneNumber,
    ...
    if (checkoutResult) {
      ...
      if (selectedPaymentMethod === 'pix') {
        startPaymentPolling(checkoutResult.pixChargeId);
      } else {
        startPaymentPolling(checkoutResult.chargeId);
      }
    }
  };
  ...
  // Close modal when payment is confirmed (handled by the hook)
  useEffect(() => {
    if (!open) {
      setCurrentStep('form');
      clearCheckoutData();
    }
  }, [open, clearCheckoutData]);
      ...
      <Tabs value={selectedPaymentMethod} onValueChange={(value) => setSelectedPaymentMethod(value as any)}>
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="pix" className="flex items-center gap-2">
            ...
            PIX
            <Badge variant="secondary" className="ml-1">-{Math.round(((checkoutData.regularPrice - checkoutData.pixPrice) / checkoutData.regularPrice) * 100)}%</Badge>
          </TabsTrigger>
              ...
              {checkoutData.pixQrCode && (
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <div className="text-center text-sm text-muted-foreground">
                    ...
                    <p>Pagamento é processado em tempo real</p>
                  </div>
                </div>
              )}
            </CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground">
                  Vencimento: {new Date(checkoutData.charge.dueDate).toLocaleDateString('pt-BR')}
                </div>
              </div>
        ...
        {currentStep === 'form' ? (
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleCreateCheckout)} className="space-y-6">
              {/* Coupon Section - same as original */}
              ...
              {/* Personal Information - same as original */}
              ...
              {/* Address Information - same as original */}
              <div className="space-y-4">
