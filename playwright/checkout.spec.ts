...
test.describe('Checkout Page - Initial State', () => {
  ...
  test('should not show credit card fields when PIX is selected', async ({
    page,
  }) => {
    await page.goto('/checkout?plan=TEOT2027R2');
...
test.describe('Checkout Page - Form Validation', () => {
  test('should show validation errors for empty required fields', async ({
    ...
    await expect(
      page.getByText(/nome deve ter pelo menos 2 caracteres/i),
    ).toBeVisible();
  ...
  test('should validate phone number length', async ({ page }) => {
    ...
    await expect(
      page.getByText(/telefone deve ter pelo menos 10 dígitos/i),
    ).toBeVisible();
  });
...
test.describe('Checkout Page - Credit Card Validation', () => {
  ...
  test('should accept card numbers up to 19 digits with formatting', async ({
    page,
  }) => {
    await page.goto('/checkout?plan=TEOT2027R2');
  ...
  test('should validate card expiry date format', async ({ page }) => {
    ...
    // Should show validation error
    await expect(
      page.getByText(/data de validade inválida|use formato mm\/aa/i),
    ).toBeVisible();
  });
...
test.describe('Checkout Page - Complete Flow', () => {
  ...
  test('should complete credit card payment flow with valid data', async ({
    page,
  }) => {
    await page.goto('/checkout?plan=TEOT2027R2');
    ...
    await Promise.race([
      ...
      page
        .getByText(/erro|error|falhou|sucesso/i)
        .waitFor({ state: 'visible', timeout: 10_000 }),
    ]).catch(() => {
  ...
  test('should show credit card preview with entered data', async ({
    page,
  }) => {
    await page.goto('/checkout?plan=TEOT2027R2');
    ...
    const cardPreview = page.locator(
      '[class*="card-preview"], [class*="CreditCard"]',
    );
...
async function fillBasicFormData(page: any) {
  ...
  const addressNumberInput = page.locator(
    'input[name="addressNumber"], input[id*="addressNumber"]',
  );
